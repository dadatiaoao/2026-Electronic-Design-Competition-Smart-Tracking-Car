        ...
        if (xQueueReceive(spp_common_uart_queue, (void * )&event, (TickType_t)portMAX_DELAY))            {
            ...
            case UART_DATA:
                if (event.size) {
                    ...
                    uart_read_bytes((uart_port_t)pvParameters, ntf, event.size, portMAX_DELAY);
...
static void ble_spp_uart_init(void)
{
    ...
    uart_driver_install(UART_NUM_1, 4096, 8192, 10, &spp_common_uart_queue, 0);
    //Set UART parameters
    uart_param_config(UART_NUM_1, &uart_config);
    //Set UART pins
    uart_set_pin(UART_NUM_1, 10, 9, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    xTaskCreate(ble_server_uart_task, "uTask", 4096, (void *)UART_NUM_1, 8, NULL);
}
