...
extern volatile uint32_t point_stop_duration_ms;
...
int main(void)
{
    ...
    point_stop_duration_ms = 5000;      // 路程点停车时长（毫秒，可调整）
    ...
    static uint8_t point_stop_active = 0;      // 路程点停车状态
    static uint32_t point_stop_elapsed_ms = 0; // 路程点停车累计时间
    static int point_resume_speed = 130;       // 路程点停车后恢复速度
		printf("启动");
    while (1)
    {
				last_display++;
        ...
        // 路程点检测与控制逻辑
        // 检查路程点触发：触发后先提示，再停车等待
        for (uint8_t i = 0; i < distance_point_count; i++) {
            if (distance_points[i].triggered == 1) {
                Enable_Buzzer();  // 触发蜂鸣器
								Enable_LED();
                distance_points[i].triggered = 2;  // 标记为已处理，避免重复触发
            if (!point_stop_active && !final_stop_triggered) {
              point_resume_speed = basicspeed;
              if (point_resume_speed <= 0) {
                point_resume_speed = 120;
              }
              point_stop_active = 1;
              point_stop_elapsed_ms = 0;
              basicspeed = 0;  // 立即停车
              printf("到达路程点: %.1fcm，停车%lums\n",
                   distance_points[i].distance_cm,
                   point_stop_duration_ms);
            }
            break;
            }
        }
        ...
        // 路程点停车计时：默认5秒，到时自动恢复
        if (point_stop_active) {
          basicspeed = 0;
          point_stop_elapsed_ms += 10;  // 主循环约每10ms执行一次

          if (point_stop_elapsed_ms >= point_stop_duration_ms) {
            point_stop_active = 0;
            point_stop_elapsed_ms = 0;

            if (!distance_threshold_reached && !final_stop_triggered) {
              basicspeed = point_resume_speed;
              printf("路程点停车结束，恢复行驶，速度:%d\n", basicspeed);
            }
          }
        }
