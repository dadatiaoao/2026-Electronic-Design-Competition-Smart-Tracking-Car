...
volatile float distance_threshold_cm = 457.2f;     // 停止阈值（厘米），可调，451.2
...
volatile uint32_t point_stop_duration_ms = 5000;   // 路程点停车时长（毫秒，默认5秒）

// 路程点配置数组（可自定义修改）
distance_point_t distance_points[] = {//弧线长度修正值79.1，原125.6
	{100.0f, 0},  //100cm触发点
    {224.1f, 0},  // 225.6cm触发点
    {324.1f, 0},  // 325.6cm触发点
    {455.2f, 0},  // 451.2cm触发点
...
void pid_control(int basicspeed)
{
  ...
  {
      motor_target_set(0, 0);
            // 临时停车阶段保留速度环输出，恢复时可快速起步
      motorA_duty(0);
      motorB_duty(0);
            // 停车时清空编码器增量，避免噪声在恢复时造成误判
            Encoder_count1 = 0;
            Encoder_count2 = 0;
      return;
  }
  ...
  if(motorB_dir){motorB.now = Encoder_count2;}else{motorB.now = -Encoder_count2;}
    ...
    // 循迹路程累计逻辑。未到达阈值且在循迹模式，开始或继续累计路程
  if (xunji == 1 && !distance_threshold_reached) {
				tracking_distance_enabled = 1;//未达阈值且在循迹模式则距离累计使能
  ...
  } else if (tracking_distance_enabled && xunji == 0) {
		// 离开循迹模式时重置累计（不在在循迹模式，未使能距离累计）
				tracking_distance_cm = 0.0f;
				tracking_distance_enabled = 0;
